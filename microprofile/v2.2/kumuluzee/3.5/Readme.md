# Eclipse MicroProfile (v2.2) DDD implementation using Kumuluzee (v3.5), MySql and RabbitMQ
...
The implementation adopts a microservices based architectural style and uses the following technologies
  - Kumuluzee (v3.5) as the MicroProfile implementation
  - JPA as the Data management platform (Hibernate)
  - MySql as the data storage mechanism and
...
Kumuluzee offers first-class support for interacting with RabbitMQ Exchanges/Queues. However it is not directly integrated with 
CDI events, so this implementation does not use CDI Events, rather it directly publishes the events to the exchanges using the API
provided by Kumuluzee

The following exchange needs to be created on the RabbitMQ Server (cargoExchange -> Type (Direct))
...
Booking MS
    ...
    Run command -> java -jar bookingms-1.0.0.jar

    
    JSON Requests (Test via Postman) ->
    
    
    Cargo Booking (http://localhost:8080/serviceapi/cargobooking)
    --------------------------------------------------
    ...
    Cargo Routing (http://localhost:8080/serviceapi/cargorouting)
    --------------------------------------------------
    {
      "bookingId": "<<BookingId>>"
    }
  ...
  Routing MS
    ...
    Run command -> PORT=8081 java -jar routingms-1.0.0.jar
...
Tracking MS
    ...
    Run command -> PORT=8082 java -jar trackingms-1.0.0.jar
...
Handling MS
    ...
    Run command -> PORT=8083 java -jar handlingms-1.0.0.jar
